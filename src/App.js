...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.secondsChange, 1000)
  }
  onClickMatchCard = imageUrl => {
    ...
    this.setState({matchedImage: newImageUrl})
  }
  secondsChange = () => {
  ...
  render() {
    const {gameOver, matchedImage, category, score, timer} = this.state
    const filteredList = imagesList.filter(each => each.category === category)
    return (
      <div className="container">
        <nav className="header-container">
          <ul className="nav-list-container">
            ...
            <li className="score-timer-container">
              ...
              <div className="timer-image-count-container">
                <img
        ...
        <div className="content-container">
          {!gameOver && (
            <div className="image-tabslist-container">
              <img src={matchedImage} alt="match" className="matched-image" />
              <ul className="tabslist-container">
                {tabsList.map(eachTabText => (
                  <TabsList
                    key={eachTabText.tabId}
                    tabDetails={eachTabText}
                    onChangeActiveId={this.onChangeActiveId}
                    isActive={eachTabText.tabId === category}
                  />
                ))}
              </ul>
              <ul className="tabItem-list-container">
                {filteredList.map(eachTabItem => (
                  <TabItem
                    key={eachTabItem.id}
                    thumbnailDetails={eachTabItem}
                    onClickMatchCard={this.onClickMatchCard}
                  />
                ))}
              </ul>
            </div>
            ...
            <div className="result-container">
              ...
              >
                <img
                  ...
                  alt="reset"
                />
                <p className="restart-text">PLAY AGAIN</p>
              </button>
            </div>
          )}
        </div>
